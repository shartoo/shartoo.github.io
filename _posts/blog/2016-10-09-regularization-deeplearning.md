...
此时的正则化解为：(关于解得由来参照附录，多元线性回归问题解的推导)
...
正则化的通用方程实际上解决了两个问题。一个是确保不发生过拟合，另外一个也解决了 $X^T X$ 的奇异矩阵问题。当 m < n 时，$X^T X$ 将是一个奇异矩阵，从数学上可以证明，加上 $\lambda I$ 后，结果将是一个非奇异矩阵。
